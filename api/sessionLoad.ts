...
export default async function handler(
  ...
  try {
    ...
    switch (tipo) {
      case 'consolidado':
        resultado = await carregarDocumentoConsolidado(incluir_metadata === 'true');
        break;
        
      case 'session':
        if (!session_id || typeof session_id !== 'string') {
          return res.status(400).json({
            erro: 'session_id é obrigatório quando tipo=session'
          });
        }
      ...
      default:
        return res.status(400).json({
          erro: 'Tipo inválido. Use: consolidado, session, ou latest'
        });
    }
